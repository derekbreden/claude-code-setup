...
- **`note-inherited-fence.sh`** — also wired here, on `Task|Agent`; described under UserPromptSubmit.
...
`block-effort-estimate.sh`, `block-unwatched-wait.sh`, `block-residue.sh`, `block-underived-measurement.sh`, `note-inherited-fence.sh`, `reap-abandoned-forks.sh`, and the unwired `block-question-as-disagreement.sh` each append one JSONL line per event to `~/.claude/hooks/logs/<hook-name>.jsonl` with a `status` field identifying which code path was taken:
...
- `hooks/note-inherited-fence.sh` — inherited-fence note (PostToolUse on Task|Agent + UserPromptSubmit on task-notification turns, once per session; inert while `calibration/Fences.md` is absent)
