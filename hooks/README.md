...
- **`block-unpriced-impossibility.sh`** — catches limit claims about a searched space — "impossible", "no room", "fully pinned", "the only option", "an envelope conversation", "FULL" — when the claim names only the pins of the current arrangement, assigns the block to something fixed (the envelope), or hands the problem upstairs, without naming what would have to move, what moving it costs, or the search box it ran. Haiku splits a BARE claim from a PRICED one (a claim carrying its move-and-cost or its bounds passes; a limit about something other than a searched space also passes). The block message asks for the pricing, not the retraction, and points at `calibration/Fences.md` — *The route as requirement*. **Also wired on SubagentStop**, so a stint's final report is asked to price its own fences before the manager inherits them. Fires only when the Fences calibration exists at its expected path; bails silently anywhere else.
