...
- **`block-unlooked-move.sh`** — in a turn that edited the enclosure's placement (`_contents.py`) or routing (`_lines.py`), blocks unless the last look is newer than those files' newest mtime anywhere in the repo. A look is a `render-view.js` or `look.sh` run, or a `Read` of a `.png`. The mtime is the authority and the transcript carries only this session, so a look taken before **any** agent's edit reads as stale — several sessions share one working tree, and a body moves under a render already taken. A session that edited neither file passes, so another agent's edit alone never blocks a turn. No Haiku stage: the markers are a file path, a tool name and a timestamp. Two block messages, split on whether a look happened at all; the stale half says its piece once a session, where the never-looked half repeats. Each carries the `tools/look.sh <body>` invocation, names the elevations the assembly builds beside its STEP, and points at `calibration/Fences.md`. The message names the build when the exported `.step` predates the edit — the renderer reads that `.step`, so a look before a build carries the previous geometry — and it names the way past for an edit that moved no geometry, which a comment, a rename, a revert and a registry note all are. Scoped to a repo carrying `tools/render/render-view.js`; silent elsewhere. `stop_hook_active` holds a turn to one block. 0.4 s against a 64 MB transcript.
...
- `already_warned_this_session` — the stale-look half already fired in this session (`block-unlooked-move.sh`)
- `no_render_repo` / `no_placement_edits` / `looked` — outside a repo with the renderer, no placement or routing edit in this session, or the last look is newer than the source (`block-unlooked-move.sh`; the `looked` and `blocked` lines carry `edits`, `looks`, `last_look` and `src_mtime`, so a block can be read back against which write beat the look)
