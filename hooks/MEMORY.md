Claude Code hooks: three Stop hooks (effort estimates, unexplained hedges, questions-as-disagreement) plus two PreToolUse hooks (residue in new file content; underived measurements in new prose).
...
`block-residue.sh` and `block-underived-measurement.sh` are PreToolUse hooks, two-stage (regex + Haiku) like the Stop hooks: they judge the new content of a write, nudge once per session via a marker in `~/.claude/hooks/state/`, and log to their own JSONL. `block-underived-measurement.sh` reduces `.py`/`.scad` to comment text and strips existing `[value](TAG)` markers before matching, and applies only inside a repo carrying `tools/docgen`.
